...
let map2_local ~f a b =
  let la = length a in
  let lb = length b in
  if la <> lb
  then invalid_arg "Iarray.map2_local: arrays must have the same length"
  else unsafe_init_local la (fun i -> f (unsafe_get a i) (unsafe_get b i))
;;
...
let map2_local_first_input_and_output ~f a b =
  let la = length a in
  let lb = length b in
  if la <> lb
  then invalid_arg "Iarray.map2_local: arrays must have the same length"
...
let map2_local_second_input_and_output ~f a b =
  let la = length a in
  let lb = length b in
  if la <> lb
  then invalid_arg "Iarray.map2_local: arrays must have the same length"
...
let fold_left_local ~f ~init:x a =
  let len = length a in
  let rec go r i = if i = len then r else go (f r (unsafe_get a i)) (i + 1) in
  go x 0
;;
...
let fold_left_local_output ~f ~init:x a =
  let len = length a in
  let rec go r i = if i = len then r else go (f r (unsafe_get a i)) (i + 1) in
  go x 0
;;
...
let fold_left_map_local ~f ~init:acc input_array =
  let len = length input_array in
  if len = 0
...
let fold_left_map_local_output ~f ~init:acc input_array =
  let len = length input_array in
  if len = 0
  then acc, unsafe_of_local_array [||]
  else (
...
let fold_right_local ~f a ~init:x =
  let rec go r i = if i = -1 then r else go (f (unsafe_get a i) r) (i - 1) in
  go x (length a - 1)
;;
...
let fold_right_local_output ~f a ~init:x =
  let rec go r i = if i = -1 then r else go (f (unsafe_get a i) r) (i - 1) in
  go x (length a - 1)
;;
