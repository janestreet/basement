...
module Atomic = struct
  module Safe = struct
    external make : 'a -> 'a Atomic.t = "%makemutable"
    external make_contended : 'a -> 'a Atomic.t = "caml_atomic_make_contended"
    external get : 'a Atomic.t -> 'a = "%atomic_load"
    external set : 'a Atomic.t -> 'a -> unit = "caml_atomic_set_stub"
    external exchange : 'a Atomic.t -> 'a -> 'a = "%atomic_exchange"
    external compare_and_set : 'a Atomic.t -> 'a -> 'a -> bool = "%atomic_cas"
    ...
    external add : (int Atomic.t[@local_opt]) -> int -> unit = "caml_atomic_add_stub"
    external sub : (int Atomic.t[@local_opt]) -> int -> unit = "caml_atomic_sub_stub"
    external logand : (int Atomic.t[@local_opt]) -> int -> unit = "caml_atomic_land_stub"
    external logor : (int Atomic.t[@local_opt]) -> int -> unit = "caml_atomic_lor_stub"
    external logxor : (int Atomic.t[@local_opt]) -> int -> unit = "caml_atomic_lxor_stub"
